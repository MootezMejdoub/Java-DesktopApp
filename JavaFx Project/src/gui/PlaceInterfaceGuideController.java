...
import gui.PlacePaneGuideController;
 ...
 * @author bahe
 */
public class PlaceInterfaceGuideController {
    @FXML
    private VBox PlaceVboxOfGuide;
    @FXML
    ...
    public void setPlacesList( ActionEvent event ) throws IOException, SQLException{
        ...
        for (final Place place : place_service.readAll()) {
            FXMLLoader load = new FXMLLoader(getClass().getResource("PlacePaneGuide.fxml"));
            Pane PlacePane= load.load();
            PlacePaneGuideController PPGC = load.getController();
//Place(String nom, double note, String address, String description, String type)
            PPGC.setPlace(place.getNom(),
                         Double.toString(place.getNote()),
                         place.getAddress(),
                         place.getDescription(),
                         place.getType(),
                         Integer.toString(place.getId()));
            PlaceVboxOfGuide.getChildren().add(PlacePane);
            System.out.print("hi");
        }
    }
